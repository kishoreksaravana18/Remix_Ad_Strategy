    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          ...
          <div className="text-3xl font-bold text-white">{report?.kpis?.competitorsCount ?? 0}</div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-zinc-500 text-sm font-medium mb-1">Keywords Mined</div>
          <div className="text-3xl font-bold text-white">{report?.kpis?.keywordsCount ?? 0}</div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-zinc-500 text-sm font-medium mb-1">Audience Segments</div>
          <div className="text-3xl font-bold text-white">{report?.kpis?.segmentsCount ?? 0}</div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-zinc-500 text-sm font-medium mb-1">Ad Copy Variants</div>
          <div className="text-3xl font-bold text-white">{report?.kpis?.copyVariantsCount ?? 0}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 space-y-4">
          ...
          <p className="text-zinc-300 leading-relaxed">{report?.offerAnalysis?.valueProp ?? 'Analyzing...'}</p>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 space-y-4">
        ...
        <div className="text-zinc-300 leading-relaxed font-mono text-sm whitespace-pre-wrap">
          {typeof offerDetails.pageSpeedHeuristics === 'string' 
            ? offerDetails.pageSpeedHeuristics 
            : JSON.stringify(offerDetails.pageSpeedHeuristics, null, 2)}
        </div>
      </div>
    </div>
  );
                  ...
                  <div className="flex-1 h-2 bg-zinc-800 rounded-full overflow-hidden">
                    <div 
                      className={`h-full rounded-full ${
                        (comp.strength ?? 0) > 75 ? 'bg-red-500' : (comp.strength ?? 0) > 40 ? 'bg-amber-500' : 'bg-emerald-500'
                      }`}
                      style={{ width: `${comp.strength ?? 0}%` }}
                    />
                  </div>
                  <span className="text-xs font-mono text-zinc-500 w-8">{comp.strength ?? 0}</span>
                </div>
              </td>
              <td className="py-4">
...
function AdCopyTab({ adCopy }: { adCopy: StrategyReport['adCopy'] }) {
  ...
  if (!currentSet) return <div className="text-zinc-500">No ad copy generated.</div>;
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          <div>
            <h3 className="text-sm font-medium text-zinc-500 uppercase tracking-wider mb-3">Headlines</h3>
          ...
          <div>
            <h3 className="text-sm font-medium text-zinc-500 uppercase tracking-wider mb-3">Descriptions</h3>
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
            ...
            <p className="text-white">{strategy?.geoPriority ?? 'Worldwide'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
          ...
          <div className="space-y-3">
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="font-mono text-white">{strategy?.deviceBids?.mobile ?? 0}%</span>
              </div>
              <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
                <div className="h-full bg-blue-500 rounded-full" style={{ width: `${Math.max(10, (strategy?.deviceBids?.mobile ?? 0) + 50)}%` }} />
              </div>
            </div>
            <div>
              <div className="flex justify-between text-sm mb-1">
                <span className="flex items-center gap-2 text-zinc-300"><Monitor className="w-4 h-4" /> Desktop</span>
                <span className="font-mono text-white">{strategy?.deviceBids?.desktop ?? 0}%</span>
              </div>
              <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
                <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${Math.max(10, (strategy?.deviceBids?.desktop ?? 0) + 50)}%` }} />
              </div>
            </div>
            <div>
              <div className="flex justify-between text-sm mb-1">
                <span className="flex items-center gap-2 text-zinc-300"><Tablet className="w-4 h-4" /> Tablet</span>
                <span className="font-mono text-white">{strategy?.deviceBids?.tablet ?? 0}%</span>
              </div>
              <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
                <div className="h-full bg-zinc-500 rounded-full" style={{ width: `${Math.max(10, (strategy?.deviceBids?.tablet ?? 0) + 50)}%` }} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
          <div className="p-4 bg-red-500/10 border-b border-zinc-800">
            ...
            <p className="text-sm text-zinc-300">{strategy?.losersVsEdge?.losers ?? 'No data'}</p>
          </div>
          <div className="p-4 bg-emerald-500/10">
            ...
            <p className="text-sm text-zinc-300">{strategy?.losersVsEdge?.edge ?? 'No data'}</p>
          </div>
        </div>
      </div>
    </div>
